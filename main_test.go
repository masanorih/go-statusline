...
func TestColorizeUsage(t *testing.T) {
	// バー幅20文字、シェードブロックによる粒度改善
	// 完全ブロック: █, シェード: ▓(75-99%), ▒(50-74%), ░(25-49%)
	...
	tests := []struct {
		...
		{"1%は緑でシェードなし", 1.0, "\033[32m1.0% [                    ]\033[0m"},           // 0.2 < 0.25
		{"2%は緑で░シェード", 2.0, "\033[32m2.0% [░                   ]\033[0m"},             // 0.4 >= 0.25
		{"3%は緑で▒シェード", 3.0, "\033[32m3.0% [▒                   ]\033[0m"},             // 0.6 >= 0.50
		{"4%は緑で▓シェード", 4.0, "\033[32m4.0% [▓                   ]\033[0m"},             // 0.8 >= 0.75
